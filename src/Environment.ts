...
import { DEFAULT_SECRETS_LISTENING_PORT, DEFAULT_SECRETS_SERVER_HOST_NAME } from "./utils/constants";

const accessAsync = promisify(access);

if (!(args as any).config.name) {
  args
    .option(["", "secretsHost"], "Optional. Customize listening address for secrets", DEFAULT_SECRETS_SERVER_HOST_NAME)
    .option(["", "secretsPort"], "Optional. Customize listening port for secrets", DEFAULT_SECRETS_LISTENING_PORT)
    .option(
      ["l", "location"],
    ...
    .option(["", "disableSoftDelete"], "Optional. Allows instant permanent deletion of key vault objects.")
    .option(
...
export default class Environment implements IEnvironment {
  ...
  public recoverableDays(): number {
    if (this.flags.recoverableDays < 7 || this.flags.recoverableDays > 90) {
      throw RangeError(`Must provide a value between 7 and 90 for parameter --recoverableDays`);
    }
